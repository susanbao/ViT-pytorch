...
from utils.data_utils_resnet_region_feature import get_loader_resnet_region_feature, tensor_ordinal_to_float, tensor_ordinal_to_float_patch
...
def setup(args):
    ...
    model = ResNetRegression(input_channels = 25, output_size = args.ordinal_class_num)
...
def valid(args, model, writer, test_loader, test_datasets, global_step):
    ...
    for step, batch in enumerate(epoch_iterator):
        ...
        with torch.no_grad():
            ...
            region_class = region_logits.argmax(dim=1).to(torch.float)
            eval_loss = loss_fct(region_class, y)
            all_preds.extend(tensor_ordinal_to_float(region_logits, test_datasets.conv_values_patch).tolist())
...
def train(args, model):
    ...
    train_loader, test_loader, train_datasets, test_datasets = get_loader_resnet_region_feature(args)
