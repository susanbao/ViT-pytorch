...
def train(args, model):
    ...
    _, test_loader, _, test_datasets = get_loader_feature(args)

    model.zero_grad()
    set_seed(args)  # Added here for reproducibility (even between python 2 and 3)
    
    model_checkpoint = f"./output/{args.name}_checkpoint_{args.checkpoint_step}.bin"
...
def main():
    ...
    parser.add_argument("--checkpoint_step", default=5000, type=int,
                        help="restore model for checkpoint steps.")
