...
def get_loader_at(args):
    ...
    split = "val"
    store_preprocess_inputs_path = model_data_path + split + f"/pre_data/{split}_{data_name}_inputs_0.npy"
    store_preprocess_annotations_path = model_data_path + split + f"/pre_data/{split}_{data_name}_annotations_0.npy"
    ...
    test_sampler = SequentialSampler(train_datasets)
    ...
    test_loader = DataLoader(train_datasets,
                             sampler=test_sampler,
                             batch_size=args.eval_batch_size,
                             num_workers=4,
                             pin_memory=True) if train_datasets is not None else None
